...
u8 COM0_RX_BUF[COM0_REC_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
u16 COM0_RX_STA=0;       //接收状态标记
...
void COM0_SendStr(HT_USART_TypeDef* USARTx, uint8_t *str)
{
	...
	for(i = 0;str[i] != '\0';i++)
	{
		USART_SendData(COM0_PORT, str[i]);
		// 等待发送数据寄存器清空
		while (USART_GetFlagStatus(COM0_PORT, USART_FLAG_TXDE) == RESET);
	}
}
...
//usmart支持部分
//将收到的AT指令应答数据返回给电脑串口
//mode:0,不清零COM0_RX_STA;
//     1,清零COM0_RX_STA;
void atk_8266_at_response(u8 mode)
{
	if(COM0_RX_STA&0X8000)		//接收到一次数据了
	{ 
		COM0_RX_BUF[COM0_RX_STA&0X7FFF]=0;//添加结束符
		printf("%s",COM0_RX_BUF);	//发送到串口
		if(mode)COM0_RX_STA=0;
	} 
}
...
u8* atk_8266_check_cmd(u8 *str)
{
	...
	{ 
		COM0_RX_BUF[COM0_RX_STA&0X7FFF]=0;//添加结束符
		strx=strstr((const char*)COM0_RX_BUF,(const char*)str);
	} 
	return (u8*)strx;
}
//向ATK-ESP8266发送命令
...
u8 atk_8266_send_cmd(u8 *cmd,u8 *ack,u16 waittime)
{
	...
	if(ack&&waittime)		//需要等待应答
	{
		...
		{
			delay_ms(10);
			if(COM0_RX_STA&0X8000)//接收到期待的应答结果
			{
...
{
	while((COM0_PORT->SR&0X40)==0);	//等待发送空
	COM0_PORT->DR='+';      
